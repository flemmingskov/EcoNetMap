"""
coTopo - Sampling Diagnostics Module
========================================
...
Part of the coTopo toolkit (Diagnostic Tool)
...
# Page configuration
st.set_page_config(
    page_title="Sampling Diagnostics - coTopo",
    page_icon="🔍",
    layout="wide"
)
...
def detect_columns(df: pd.DataFrame) -> Dict[str, str]:
    """Detect correct column names in dataframe"""
    columns = {}
    
    # Year column
    if 'year' in df.columns:
        columns['year'] = 'year'
    elif 'aarstal' in df.columns:
        columns['year'] = 'aarstal'
    else:
        st.warning("Could not find year column (tried: 'year', 'aarstal')")
    
    # Plot ID column
    if 'aktId' in df.columns:
        columns['plot_id'] = 'aktId'
    elif 'plotId' in df.columns:
        columns['plot_id'] = 'plotId'
    else:
        st.warning("Could not find plot ID column (tried: 'aktId', 'plotId')")
    
    # Species column - try multiple possibilities
    species_candidates = ['videnskabeligtNavn', 'species', 'scientificName', 'species_name', 'almindeligtNavn']
    for col in species_candidates:
        if col in df.columns:
            columns['species'] = col
            break
    if 'species' not in columns:
        st.warning(f"Could not find species column (tried: {species_candidates})")
    
    # Habitat/nature type column
    habitat_candidates = ['naturtypeId', 'naturtype', 'habitat', 'habitatType', 'vegetationType']
    for col in habitat_candidates:
        if col in df.columns:
            columns['habitat'] = col
            break
    ...
    # Taxon ID for record counting
    if 'taxonId' in df.columns:
        columns['taxon_id'] = 'taxonId'
...
if selected_source == "All data (session)":
    df = st.session_state.df_vegetation.copy()
...
if 'aarstal' in df.columns and 'year' not in df.columns:
    df['year'] = df['aarstal']
elif 'year' not in df.columns and 'aarstal' not in df.columns:
    st.error("No year column found (looking for 'aarstal' or 'year')")
    st.stop()
...
if habitat_temporal is not None and len(habitat_temporal) > 0:
    ...
    ax.set_xlabel('Year', fontsize=12)
    ax.set_ylabel('Habitat Type (naturtypeId)', fontsize=12)
    ...
    st.info("No habitat information (naturtypeId) found in dataset")
...
# Footer
st.markdown("---")
st.caption("coTopo Sampling Diagnostics | Flemming Skov | Aarhus University | 2026")
