"""
coTopo - Reference Map Validation Analysis (Corrected Final Version)
=========================================================================
...
def load_overlay_db(path: Path) -> dict:
    ...
    try:
        ...
        if 'aktId' in tables:
            result['aktId'] = pd.read_sql_query("SELECT * FROM aktId", conn)
...
def calc_distance_preservation_global(aktId: pd.DataFrame, use_mantel: bool = True) -> dict:
    """
    ...
    Args:
        aktId: Plot-level data with coordinates and indicators
        use_mantel: If True, use Mantel test (with permutations); if False, use simple Pearson
    """
    plots = aktId[['xcoor', 'ycoor']].dropna()
    ...
    indicators = ['L', 'M', 'R', 'N']
    available_indicators = [ind for ind in indicators if ind in aktId.columns]

    if len(available_indicators) < 2:
        return None

    plot_features = aktId.loc[plot_indices, available_indicators].dropna()
    # Re-align: only keep plots that have all indicator values
    valid_indices = plot_features.index.tolist()
    if len(valid_indices) < 10:
        return None
    coords = aktId.loc[valid_indices, ['xcoor', 'ycoor']].values
    euclidean_dists = pdist(coords, metric='euclidean')
...
def calc_distance_preservation_spatial(aktId: pd.DataFrame, n_regions=16) -> dict:
    """
    ...
    for region in range(16):
        ...
        if len(region_indices) >= 10:
            ...
            region_features = aktId.loc[region_indices, available_indicators].dropna()
            valid_region_indices = region_features.index.tolist()
            
            if len(valid_region_indices) < 10:
                continue
                
            region_coords = aktId.loc[valid_region_indices, ['xcoor', 'ycoor']].values
...
def cross_validate_predictions(aktId: pd.DataFrame, indicator: str, 
    ...
    """
    data = aktId[['xcoor', 'ycoor', indicator]].dropna()
...
def analyze_habitat_differentiation(aktId: pd.DataFrame, 
                                    habitat_col: str,
                                    indicators: list = ['M', 'L', 'N', 'R']) -> dict:
    """
    Test if habitat types differ in environmental conditions using ANOVA.
    
    Uses plot-level data grouped by habitat classification (naturtypeId or major_type).
    Tests whether field-classified habitats correspond to environmental differences.
    """
    if habitat_col not in aktId.columns:
        return None
    
    # Get plots with habitat classification
    data = aktId[[habitat_col] + indicators].dropna(subset=[habitat_col])
...
def analyze_habitat_spatial_clustering(aktId: pd.DataFrame, 
    ...
    data = aktId[[habitat_col, 'xcoor', 'ycoor']].dropna()
...
if st.button("🔬 Run Comprehensive Validation", type="primary", use_container_width=True):
            ...
            with st.spinner("Calculating Procrustes similarities..."):
                for i in range(n_overlays):
                    for j in range(i + 1, n_overlays):
                        ...
                        taxa_i = taxa_i[taxa_i['artId'].isin(common_species)].sort_values('artId')
                        
                        taxa_j = data[names[j]]['taxa']
                        taxa_j = taxa_j[taxa_j['artId'].isin(common_species)].sort_values('artId')
            ...
            if mds_maps:
                ...
                with st.spinner("Calculating MDS stress..."):
                    for name in mds_maps:
                        if data[name].get('aktId') is None:
                            continue

                        aktId = data[name]['aktId']
                        plots = aktId[['xcoor', 'ycoor']].dropna()
                        ...
                        if len(available_indicators) >= 2:
                            ...
                            coords_2d = aktId.loc[valid_stress_indices, ['xcoor', 'ycoor']].values
        ...
        with st.spinner("Calculating gradients..."):
            for name, d in data.items():
                ...
                for indicator in test_indicators:
                    if indicator in aktId.columns:
                        grad = calc_gradient(aktId, indicator)
                        if grad:
            ...
            if do_null_models and gradient_results:
                ...
                if null_test_map:
                    with st.spinner(f"Running permutation test for {null_test_map} (999 permutations)..."):
                        ...
                        for indicator in test_indicators:
                            if indicator in data[null_test_map]['aktId'].columns:
                                null_res = calc_gradient_null_model(
                                    data[null_test_map]['aktId'],
                                    indicator,
                                    n_permutations=999
                                )
                                if null_res:
    ...
    if do_distance:
        ...
        with st.spinner("Analyzing distance preservation..."):
            for name, d in data.items():
                ...
                result_global = calc_distance_preservation_global(d['aktId'], use_mantel=True)
                if result_global:
                ...
                # Spatial
                result_spatial = calc_distance_preservation_spatial(d['aktId'])
                if result_spatial:
    ...
    if do_prediction:
        ...
        for name, d in data.items():
            ...
            for indicator in pred_indicators:
                if indicator in d['aktId'].columns:
                    progress_current += 1
    ...
    if do_habitat:
        ...
        with st.expander("ℹ️ Method Explanation", expanded=False):
            ...
            **This tests:** Do your habitat classifications (naturtypeId or major_type) 
                ...
                **Interpretation:** Lower variance indicates tighter spatial clustering. 
                ...
                if n_habitats_total <= 12:
                    ...
                    **Points:** Centroids (size = n plots) | **Small dashed circles:** Spread indicator (scaled)  
                    ...
                    Consider using **major_type** instead of **naturtypeId** for clearer spatial patterns.
                    """)
...
<div style='text-align: center; color: #7f8c8d; font-size: 0.9em;'>
    coTopo - Comprehensive Validation | Network-Based Ecological Cartography
</div>
""", unsafe_allow_html=True)
