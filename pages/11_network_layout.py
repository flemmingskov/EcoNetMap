"""
coTopo - Network Analysis & Reference Map Generation
=======================================================
...
Part of the coTopo toolkit (Graph Construction & Reference Map 1/3)
...
# Page configuration
st.set_page_config(
    page_title="Network Analysis - coTopo", 
    page_icon="🕸️",
    layout="wide"
)
...
def create_species_nodes(df: pd.DataFrame, min_occurrences: int = 1) -> pd.DataFrame:
    """Create node list from species data with minimum occurrence filter"""
    species_counts = df.groupby('artId').size().reset_index(name='occurrence_count')
    species_info = df[['artId', 'almindeligtNavn']].drop_duplicates()
    species_info = species_info.rename(columns={'almindeligtNavn': 'label'})
    species_nodes = species_info.merge(species_counts, on='artId', how='left')
    ...
    species_nodes_filtered = species_nodes_filtered.sort_values('artId').reset_index(drop=True)
    return species_nodes_filtered

def create_co_occurrence_edges_jaccard(df: pd.DataFrame, valid_species: set, 
                                       min_jaccard: float = 0.0) -> pd.DataFrame:
    """
    ...
    df_filtered = df[df['artId'].isin(valid_species)].copy()
    ...
    with st.spinner("Calculating Jaccard similarity for species pairs..."):
        ...
        species_occurrences = df_filtered.groupby('artId')['aktId'].nunique().to_dict()
        
        # Step 2: Count co-occurrences (intersection)
        co_occurrence = df_filtered.groupby('aktId')['artId'].unique()
...
def create_graph_from_data(nodes_df: pd.DataFrame, edges_df: pd.DataFrame) -> Optional[ig.Graph]:
    ...
    try:
        ...
        # Add vertices
        artIds = nodes_df['artId'].tolist()
        labels = nodes_df['label'].tolist()
        g.add_vertices(len(artIds))
        ...
        # Map artIds to vertex indices
        artId_to_index = {artId: i for i, artId in enumerate(artIds)}
        
        # Convert edge artIds to vertex indices
        edges_indices = []
        weights = []
        for _, edge in edges_df.iterrows():
            try:
                idx1 = artId_to_index[edge['source']]
                idx2 = artId_to_index[edge['target']]
...
tab1, tab2 = st.tabs([
    "📁 1. Load & Configure Network", 
    "🎯 2. Generate, Review & Save"
])
...
with tab1:
        ...
        with st.spinner(f"Loading {selected_db}..."):
            ...
            if df is not None:
                ...
                st.session_state.db_load_metrics = {
                    'records': len(df),
                    'species': df['artId'].nunique() if 'artId' in df.columns else 'N/A',
                    'plots':   df['aktId'].nunique() if 'aktId' in df.columns else 'N/A',
                }
    ...
    if st.session_state.network_data is not None:
        ...
        **Configure network construction parameters:**
        ...
        with col1:
            species_counts = df.groupby('artId').size()
        ...
        with col3:
            st.markdown("**Preview:**")
            total_species = df['artId'].nunique()
        ...
        if st.button("🔨 Build Network with Jaccard Index", type="primary", use_container_width=True):
            with st.spinner("Creating node list..."):
                nodes_df = create_species_nodes(df, min_occurrences)
            ...
            with st.spinner("Calculating Jaccard similarities..."):
                valid_species = set(nodes_df['artId'])
                edges_df = create_co_occurrence_edges_jaccard(df, valid_species, min_jaccard)
            ...
            if len(edges_df) > 0:
                ...
                with col1:
                    ...
                    species_names = dict(zip(nodes_df['artId'], nodes_df['label']))
...
st.markdown(
    ...
    coTopo - Network-based Reference Map Generation
    </div>
    """, 
    unsafe_allow_html=True
)
