...
# Page configuration
st.set_page_config(
    page_title="Species Mapping - coTopo", 
    page_icon="🗺️",
    layout="wide"
)
...
def safe_merge_data(left_df: pd.DataFrame, right_df: pd.DataFrame, on_col: str, 
    ...
    """Safely merge dataframes and handle duplicate aarstal and naturtypeId columns"""
    try:
        if required_cols:
        ...
        # Handle aarstal column specifically - prefer plot data and rename to standard name
        if 'aarstal_plot' in result.columns:
            result['aarstal'] = result['aarstal_plot']
            # Remove the suffixed versions
            result = result.drop([col for col in result.columns if col.endswith('_occ') and col.startswith('aarstal')], axis=1)
            result = result.drop([col for col in result.columns if col.endswith('_plot') and col.startswith('aarstal')], axis=1)
        elif 'aarstal_occ' in result.columns:
            result['aarstal'] = result['aarstal_occ']
            result = result.drop([col for col in result.columns if col.endswith('_occ') and col.startswith('aarstal')], axis=1)
        
        # Handle naturtypeId column the same way - prefer plot data
        if 'naturtypeId_plot' in result.columns:
            result['naturtypeId'] = result['naturtypeId_plot']
            result = result.drop([col for col in result.columns if col.endswith('_occ') and col.startswith('naturtypeId')], axis=1)
            result = result.drop([col for col in result.columns if col.endswith('_plot') and col.startswith('naturtypeId')], axis=1)
        elif 'naturtypeId_occ' in result.columns:
            result['naturtypeId'] = result['naturtypeId_occ']
            result = result.drop([col for col in result.columns if col.endswith('_occ') and col.startswith('naturtypeId')], axis=1)
...
def get_species_name_variants(species_info: pd.DataFrame) -> Dict[str, str]:
    ...
    names = {}
    name_columns = ['keyword', 'almindeligtNavn', 'videnskabeligtNavn', 'scientificName', 
...
def calculate_ellenberg_changes(early_species, late_species, aktId_df):
    """Calculate changes in Ellenberg indicator values with significance testing"""
    # Merge with aktId data to get Ellenberg values
    early_ellenberg = early_species.merge(
        aktId_df[['aktId', 'L', 'M', 'N', 'R', 'T']], 
        on='aktId', 
        how='left'
    )
...
def get_species_occurrences(occurrence_df: pd.DataFrame, selected_species: str) -> Tuple[pd.DataFrame, Optional[str]]:
    ...
    for col in ['almindeligtNavn', 'keyword', 'species_name']:
...
if 'map_data' in st.session_state:
    try:
        ...
        plot_df = map_data.get('aktId', pd.DataFrame())
        ...
        with tab1:
            try:
                ...
                if occurrence_df.empty:
                    ...
                    st.info("This tab requires an 'aktId' table with plot information")
                else:
                    ...
                    if not species_list:
                        ...
                        if species_info.empty:
                            ...
                            # Create display name
                            scientific_name = species_names.get('videnskabeligtNavn') or species_names.get('scientificName')
                            if scientific_name:
                            ...
                            if species_occurrences.empty:
                                ...
                                if not available_merge_cols:
                                    ...
                                    has_temporal = 'aarstal' in plot_df.columns
                                    if has_temporal:
                                        available_merge_cols.append('aarstal')
                                    
                                    # Add naturtypeId for hover info
                                    if 'naturtypeId' in plot_df.columns:
                                        available_merge_cols.append('naturtypeId')
                                    
                                    species_data = safe_merge_data(
                                        species_occurrences, plot_df, 'aktId', available_merge_cols
                                    ...
                                    if species_data.empty:
                                        ...
                                        if species_data.empty:
                                            ...
                                            temporal_cols = [col for col in species_data.columns if 'aarstal' in col.lower()]
                                            ...
                                            # Check if we have temporal data (look for any aarstal column)
                                            temporal_col = None
                                            if 'aarstal' in species_data.columns:
                                                temporal_col = 'aarstal'
                                            elif temporal_cols:  # Use first temporal column found
                                                temporal_col = temporal_cols[0]
                                                species_data['aarstal'] = species_data[temporal_col]  # Standardize name
                                                temporal_col = 'aarstal'
                                            ...
                                            if selected_period_data.empty:
                                                ...
                                                with st.expander("Species distribution across habitat types", expanded=False):
                                                    ...
                                                    try:
                                                        # Check if 'naturtypeId' is already in selected_period_data
                                                        if 'naturtypeId' in selected_period_data.columns:
                                                            habitat_data = selected_period_data.copy()
                                                        elif 'naturtypeId' in plot_df.columns:
                                                            # Merge only if needed
                                                            habitat_data = selected_period_data.merge(
                                                                plot_df[['aktId', 'naturtypeId']],
                                                                on='aktId',
                                                                how='left'
                                                            )
                                                        
                                                        if not habitat_data.empty and 'naturtypeId' in habitat_data.columns:
                                                            # Count occurrences per habitat type
                                                            habitat_counts = (
                                                                habitat_data['naturtypeId']
                                                                .dropna()
                                                                .value_counts()
                                                                .reset_index()
                                                            )
                                                ...
                                                with st.expander("Species geographic distribution", expanded=False):
                                                    ...
                                                    if validate_required_columns(plot_df, ['UTMx', 'UTMy']):
                                                        # Get geographic data for the selected period
                                                        geo_data = selected_period_data.dropna(subset=['UTMx', 'UTMy'])
                                                        ...
                                                        if len(geo_data) > 0:
                                                            ...
                                                            use_plotly = st.radio(
                                                                ...
                                                                index=0,
                                                                key="geo_map_type",
                                                                help="Matplotlib provides geographic basemap, Plotly provides hover info (aktId, naturtypeId)"
                                                            )
                                                            ...
                                                            if use_plotly_map:
                                                                ...
                                                                if show_all_plots:
                                                                    all_geo = plot_df.dropna(subset=['UTMx', 'UTMy'])
                                                                    if len(all_geo) > 0:
                                                                ...
                                                                # Build hover text for Survey plots with aktId and naturtypeId
                                                                hover_columns = ['aktId']
                                                                if 'naturtypeId' in geo_data.columns:
                                                                    hover_columns.append('naturtypeId')
                                                                ...
                                                                if show_regional_records and not df_regional.empty:
                                                                    ...
                                                                    latin_name = species_names.get('videnskabeligtNavn') or species_names.get('scientificName')
                                                                    
                                                                    regional_species = pd.DataFrame()
                                                                    
                                                                    if latin_name and 'species' in df_regional.columns:
                                                                        # Match using the Latin name against 'videnskabeligtNavn' column
                                                                ...
                                                                with col2:
                                                                    if st.button("💾 Save Map", key="save_btn_geo_species"):
                                                                        ...
                                                                        if show_all_plots:
                                                                            ...
                                                                            if len(all_geo) > 0:
                                                                                ax_save.scatter(
                                                                                    all_geo['UTMx'], all_geo['UTMy'],
                                                                                    s=20, c='lightgrey', alpha=0.3,
                                                                                    label='All plots', zorder=1
                                                                                )
                                                                        
                                                                        ax_save.scatter(
                                                                            geo_data['UTMx'], geo_data['UTMy'],
                                                        ...
                                                        st.info("Geographic coordinates (UTMx, UTMy) not available")
                                                ...
                                                with st.expander("Species in ecological space", expanded=False):
                                                    ...
                                                    if use_plotly_eco_map:
                                                        ...
                                                        if 'naturtypeId' in selected_period_data.columns:
                                                            hover_columns.append('naturtypeId')
                                                        if 'aarstal' in selected_period_data.columns:
                                                            hover_columns.append('aarstal')
                                            ...
                                            geo_data_combined = selected_period_data.dropna(subset=['UTMx', 'UTMy'])
                                            coords = selected_period_data[['xcoor', 'ycoor']].values
                                            df_regional = st.session_state.get('df_regional_pool', pd.DataFrame())
                                            ...
                                            if len(geo_data_combined) > 0:
                                                ax_geo_combined.scatter(geo_data_combined['UTMx'], geo_data_combined['UTMy'],
                        ...
                        if species_info.empty:
                            ...
                            if species_occurrences.empty:
                                ...
                                if not available_merge_cols:
                                    ...
                                    for col in ['aarstal', 'naturtypeId', 'UTMx', 'UTMy']:
                                    ...
                                    if species_data.empty:
                                        ...
                                        if species_data.empty:
                                            ...
                                            if 'aarstal' in species_data.columns:
                                                ...
                                                temporal_col = temporal_cols[0]
                                                species_data['aarstal'] = species_data[temporal_col]
                                                temporal_col = 'aarstal'
...
st.markdown(
    ...
    coTopo - Network-based ecological mapping
    </div>
    """, 
    unsafe_allow_html=True
)
