...
if 'plot_map_data' in st.session_state:
    ...
    if 'aktId' not in map_data:
        st.error("Plot data (aktId table) not found in analysis map database")
        st.stop()
...
if 'reference_map_data' in st.session_state:
    ...
    if 'aktId' in ref_data:
        ref_plot_df = ref_data['aktId']

        with st.expander(f"🗺️ Reference Map Summary: {st.session_state.get('reference_map_name', 'Unknown')}", expanded=False):
            col1, col2, col3, col4 = st.columns(4)

            with col1:
                st.metric("Total Plots", f"{ref_plot_df['aktId'].nunique():,}")
            with col2:
                if 'naturtypeId' in ref_plot_df.columns:
                    st.metric("Habitat Types", f"{ref_plot_df['naturtypeId'].nunique():,}")
                else:
                    st.metric("Habitat Types", "N/A")
            with col3:
    ...
    with tab1:
        ...
        if 'reference_map_data' in st.session_state and 'aktId' in st.session_state.reference_map_data:
            reference_df = st.session_state.reference_map_data['aktId']
        else:
        ...
        if 'aarstal' not in plot_df.columns:
            ...
            with col1:
                ...
                with st.expander("🗺️ Regional Filters", expanded=False):
                    ...
                    if 'novanareg' in plot_df.columns:
                        ...
                        filtered_plots_t2 = plot_df[plot_df['novanareg'].isin(selected_regions_t2)] if selected_regions_t2 else plot_df
                    else:
                        filtered_plots_t2 = plot_df

                    # Bioregion filter
                    if 'bioreg' in filtered_plots_t2.columns:
                        bioregions = filtered_plots_t2['bioreg'].dropna().unique()
                        ...
                        if selected_bioregions_t2:
                            filtered_plots_t2 = filtered_plots_t2[filtered_plots_t2['bioreg'].isin(selected_bioregions_t2)]
            ...
            with col2:
                ...
                with st.expander("🏞️ Vegetation & Habitat Type Filters", expanded=False):
                    ...
                    # Habitat type filter
                    if 'naturtypeId' in filtered_plots_t2.columns and 'major_type' in filtered_plots_t2.columns:
                        if selected_major_types_t2:
                            available_habitats = filtered_plots_t2[
                                filtered_plots_t2['major_type'].isin(selected_major_types_t2)
                            ]['naturtypeId'].dropna().unique()
                        else:
                            available_habitats = filtered_plots_t2['naturtypeId'].dropna().unique()
                        ...
                        if selected_habitats_t2:
                            filtered_plots_t2 = filtered_plots_t2[filtered_plots_t2['naturtypeId'].isin(selected_habitats_t2)]
            ...
            if len(filtered_plots_t2) > 0:
                # Time range settings
                min_year = int(filtered_plots_t2['aarstal'].min())
                max_year = int(filtered_plots_t2['aarstal'].max())

                if min_year < max_year:
                    # Calculate median year
                    median_year = int(filtered_plots_t2['aarstal'].median())
                    ...
                    # Split data by time
                    early_data = filtered_plots_t2[filtered_plots_t2['aarstal'] <= year_range[0]]
                    late_data = filtered_plots_t2[filtered_plots_t2['aarstal'] > year_range[1]]
