"""
coTopo - Rare Species Coordinate Assignment
==============================================================
...
# Page configuration
st.set_page_config(
    page_title="Rare Species Assignment - coTopo", 
    page_icon="🕸️",
    layout="wide"
)
...
def load_source_data(db_path: str) -> Tuple[Optional[pd.DataFrame], Dict]:
    ...
    try:
        ...
        if 'data' in tables['name'].values:
            ...
            info['species_count'] = data['artId'].nunique()
            info['plot_count'] = data['aktId'].nunique()
...
def extract_species_list(data_df: pd.DataFrame) -> pd.DataFrame:
    """Extract unique species list from data"""
    species_list = data_df.groupby('artId').agg({
        'almindeligtNavn': 'first',
        'aktId': 'nunique'
    }).reset_index()
    
    species_list.columns = ['artId', 'species_name', 'plot_count']
    return species_list
...
def compare_species_lists(source_species: pd.DataFrame, reference_coords: pd.DataFrame) -> Dict:
    ...
    source_artids = set(source_species['artId'].unique())
    reference_artids = set(reference_coords['artId'].unique())
    
    missing_in_reference = source_artids - reference_artids
    
    missing_species_df = source_species[source_species['artId'].isin(missing_in_reference)].copy()
    missing_species_df = missing_species_df.sort_values('plot_count', ascending=False)
    
    comparison = {
        'source_total': len(source_artids),
        'reference_total': len(reference_artids),
        'overlap': len(source_artids & reference_artids),
...
def calculate_jaccard_similarity(species_artid: int, positioned_artid: int, 
                                 source_data: pd.DataFrame) -> float:
    """Calculate Jaccard similarity between rare species and positioned species"""
    # Get plots for each species
    species_plots = set(source_data[source_data['artId'] == species_artid]['aktId'])
    positioned_plots = set(source_data[source_data['artId'] == positioned_artid]['aktId'])
...
def assign_coordinates_to_species(species_artid: int, 
    ...
    Parameters:
    - species_artid: Species to assign coordinates to
    - source_data: Original data used for graph construction
    - reference_coords: Species already positioned in network
    ...
    species_plots = source_data[source_data['artId'] == species_artid]['aktId'].unique()
    ...
    return {
        ...
        'neighbor_artids': [n['artId'] for n in neighbors],
...
def check_for_duplicates(conn: sqlite3.Connection, coord_table: str, 
                        new_species_artids: List[int]) -> List[int]:
    """Check for existing artIds to prevent duplicates"""
    try:
        if len(new_species_artids) == 0:
            return []
        existing_query = f"SELECT DISTINCT artId FROM {coord_table} WHERE artId IN ({','.join(map(str, new_species_artids))})"
        existing_artids = pd.read_sql_query(existing_query, conn)['artId'].tolist()
        return existing_artids
    except Exception:
        return []
...
with tab1:
    ...
    if st.button("📊 Load and Analyze Databases", type="primary", use_container_width=True):
        ...
        with st.spinner("Extracting species list..."):
            source_species = extract_species_list(source_data)
    ...
    if st.session_state.ref_coords is not None:
        ...
        if comparison['missing_in_reference'] > 0:
            with st.expander(f"🔍 Rare species needing coordinates ({comparison['missing_in_reference']:,})", expanded=False):
                ...
                st.dataframe(
                    missing_df[['artId', 'species_name', 'plot_count']],
    ...
    if st.session_state.comparison is None:
        ...
        if st.button("🎯 Assign Coordinates to Rare Species", type="primary", use_container_width=True):
            ...
            with st.expander("📋 Assignment Details", expanded=False):
                ...
                with tab1:
                    if len(successful_df) > 0:
                        display_cols = ['artId', 'keyword', 'plot_count', 'xcoor', 'ycoor', 
                ...
                with tab2:
                    if len(failed_df) > 0:
                        st.dataframe(
                            failed_df[['artId', 'keyword', 'plot_count', 'failure_reason']],
                            use_container_width=True,
                            hide_index=True
                        )
            ...
            if len(successful_assignments) == 0:
                ...
                if st.button("💾 Save Enhanced Coordinates", type="primary", use_container_width=True):
                    try:
                        ...
                        new_artids = successful_assignments['artId'].tolist()
                        existing_artids = check_for_duplicates(conn, coord_table, new_artids)
                    
                        if existing_artids:
                            st.warning(f"⚠️ Found {len(existing_artids)} species already in database - skipping duplicates")
                            successful_assignments = successful_assignments[~successful_assignments['artId'].isin(existing_artids)]
                        ...
                        for col in existing_col_names:
                            if col not in new_species.columns:
                                ...
                                elif col not in ['xcoor', 'ycoor', 'distance', 'bearing', 'artId', 'keyword', 'label']:
                        ...
                        metadata = pd.DataFrame([{
                            ...
                            'duplicates_prevented': len(existing_artids) if existing_artids else 0,
                        ...
                        with col4:
                            if existing_artids:
                                st.metric("Duplicates skipped", f"{len(existing_artids):,}")
                            else:
