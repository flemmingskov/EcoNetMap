...
Part of the coTopo toolkit (Data Management 2/2)
...
# Page configuration
st.set_page_config(
    page_title="Data Filtering - coTopo",
    page_icon="🔍",
    layout="wide"
)
...
def summarize_data(df: pd.DataFrame) -> dict:
    ...
    if 'dato' in df.columns:
        try:
            ...
            date_range = (df['dato'].min(), df['dato'].max())
        except (ValueError, TypeError, KeyError):
            date_range = (None, None)
    
    return {
        'unique_plots': df['aktId'].nunique() if 'aktId' in df.columns else 0,
        'unique_nature_types': df['naturtypeId'].nunique() if 'naturtypeId' in df.columns else 0,
        'unique_species': df['almindeligtNavn'].nunique() if 'almindeligtNavn' in df.columns else 0,
...
def safe_sample(group: pd.DataFrame, max_sample_size: int, random_seed: Optional[int] = None) -> np.ndarray:
    ...
    unique_plots = group['aktId'].unique()
...
def create_stratified_sample(df: pd.DataFrame, max_per_habitat: int, min_occurrences: int, 
    ...
    for habitat, group in df.groupby('naturtypeId'):
        sampled_plots.extend(safe_sample(group, max_per_habitat, random_seed))
    
    # Filter to sampled plots
    df_sampled = df[df['aktId'].isin(sampled_plots)]
    
    # Apply minimum occurrence filter
    if 'almindeligtNavn' in df_sampled.columns:
        species_counts = df_sampled['almindeligtNavn'].value_counts()
        species_to_keep = species_counts[species_counts >= min_occurrences].index
        df_sampled = df_sampled[df_sampled['almindeligtNavn'].isin(species_to_keep)]
...
def apply_random_sample(df: pd.DataFrame, sample_percentage: int, 
    ...
    if sample_percentage >= 100 or 'aktId' not in df.columns:
        return df
    
    if random_seed is not None:
        np.random.seed(random_seed)
    
    unique_plots = df['aktId'].unique()
    ...
    return df[df['aktId'].isin(sampled_plots)]
...
def export_to_sqlite(df: pd.DataFrame, file_path: Path, metadata: dict,
    ...
    try:
        with sqlite3.connect(file_path) as conn:
            ...
            # Table 2: 'aktId' - Unique plot information
            if 'aktId' in df.columns:
                plot_columns = ['aktId']

                standard_plot_cols = [
                    'stationsNr', 'plotNr', 'aarstal', 'progid', 'dato',
                    'naturtypeId', 'stedid', 'status', 'UTMx', 'UTMy',
                    'stednavn', 'bioreg', 'novanareg', 'major_type', 'taxonomi'
                ]

                species_specific_cols = [
                    'artId', 'almindeligtNavn', 'videnskabeligtNavn', 'species',
                ...
                for col in df.columns:
                    if (col not in plot_columns and
                        col not in species_specific_cols and
                        not col.lower().startswith('art') and
                        not col.lower().startswith('species') and
                        'navn' not in col.lower()):
                        plot_columns.append(col)

                aktId_data = df[plot_columns].drop_duplicates('aktId').reset_index(drop=True)
                aktId_data.to_sql('aktId', conn, if_exists='replace', index=False)

            # Table 3: 'species_list' - Species with occurrence counts
            if 'almindeligtNavn' in df.columns:
                species_data = df['almindeligtNavn'].value_counts().reset_index()
                species_data.columns = ['almindeligtNavn', 'occurrences']
...
with tab1:
    ...
    # Initialize data
    df_taxa_all = st.session_state.df_taxa
    df_vegetation = st.session_state.df_vegetation.copy()
    
    # Ensure proper date handling
    df_vegetation = ensure_datetime_compatibility(df_vegetation, 'dato')
    ...
    with col2:
        ...
        if 'major_type' in df_vegetation.columns and 'naturtypeId' in df_vegetation.columns:
            ...
            if selected_major_types:
                ...
                with st.expander("🔧 Select/deselect specific habitat IDs", expanded=False):
                    ...
                    for major_type in selected_major_types:
                        habitat_ids = sorted(df_vegetation[df_vegetation['major_type'] == major_type]['naturtypeId'].unique())
                        plot_counts = df_vegetation[df_vegetation['major_type'] == major_type].groupby('naturtypeId')['aktId'].nunique()
            ...
            if len(available_methods) > 0:
                with st.expander("📊 View sampling method distribution", expanded=False):
                    for method in available_methods:
                        plot_count = df_vegetation[df_vegetation[method] == 1]['aktId'].nunique() if 'aktId' in df_vegetation.columns else 0
        ...
        if 'dato' in df_vegetation.columns:
            ...
            date_range = st.date_input(
                ...
                help="Filter data by collection date",
                key="date_range_filter"
            )
            
            st.caption(f"Data available from {data_min_date} to {data_max_date}")
        else:
            st.warning("⚠️ 'dato' column not found")
            date_range = None
    ...
    with col1:
        ...
        if filter_invasive_plots and has_origin_columns:
            ...
            invasive_species_ids = df_taxa_all[df_taxa_all['isInvasiv'] == 1]['artId'].unique()
            if len(invasive_species_ids) > 0 and 'artId' in df_vegetation.columns:
                plots_with_invasives = df_vegetation[df_vegetation['artId'].isin(invasive_species_ids)]['aktId'].unique()
                st.caption(f"✅ {len(plots_with_invasives):,} plots contain invasive species ({len(plots_with_invasives)/df_vegetation['aktId'].nunique()*100:.1f}% of total)")
            else:
    ...
    if st.button("🔥 Apply All Filters", type="primary", use_container_width=True):
        with st.spinner("Applying filters..."):
            try:
                ...
                if selected_habitats and 'naturtypeId' in df_vegetation_filtered.columns:
                    df_vegetation_filtered = df_vegetation_filtered[df_vegetation_filtered['naturtypeId'].isin(selected_habitats)]
                ...
                # Apply date filter
                if date_range and 'dato' in df_vegetation_filtered.columns and len(date_range) == 2:
                    start_date, end_date = date_range
                    start_datetime = pd.to_datetime(start_date)
                    end_datetime = pd.to_datetime(end_date)
                    
                    df_vegetation_filtered = df_vegetation_filtered[
                        (df_vegetation_filtered['dato'] >= start_datetime) & 
                        (df_vegetation_filtered['dato'] <= end_datetime)
                    ]
                ...
                if filter_invasive_plots and has_origin_columns:
                    ...
                    if len(invasive_species_ids) > 0 and 'artId' in df_vegetation_filtered.columns:
                        plots_with_invasives = df_vegetation_filtered[
                            df_vegetation_filtered['artId'].isin(invasive_species_ids)
                        ]['aktId'].unique()
                        
                        df_vegetation_filtered = df_vegetation_filtered[
                            df_vegetation_filtered['aktId'].isin(plots_with_invasives)
                        ]
                ...
                if 'artId' in df_vegetation_filtered.columns and 'artId' in df_taxa_filtered.columns:
                    # Merge with FILTERED taxa (not all taxa) - this preserves taxonomic filters
                    df_merged = pd.merge(
                        df_vegetation_filtered, 
                        df_taxa_filtered[['artId', 'taxonomi']], 
                        on='artId', 
                        how='inner'
                    )
                else:
                    st.error("Cannot merge: 'artId' column missing")
                ...
                with col1:
                    st.metric(
                        "Plots Selected",
                        f"{df_merged['aktId'].nunique() if 'aktId' in df_merged.columns else 0:,}",
                        f"{df_merged['aktId'].nunique()/df_vegetation['aktId'].nunique()*100:.1f}% of total"
                    )
                
                with col2:
                    st.metric(
                        "Species Selected",
                        f"{df_merged['almindeligtNavn'].nunique() if 'almindeligtNavn' in df_merged.columns else 0:,}",
                        f"{df_merged['almindeligtNavn'].nunique()/len(df_taxa_all)*100:.1f}% of total"
                    )
...
with tab2:
    ...
    if 'df_merged' not in st.session_state:
        ...
        with col1:
            ...
            if 'almindeligtNavn' in df_current.columns:
                species_counts = df_current['almindeligtNavn'].value_counts()
            else:
        ...
        if st.button("✨ Apply Refinements", type="primary", use_container_width=True):
            with st.spinner("Applying refinements..."):
                ...
                if 'almindeligtNavn' in df_refined.columns and min_occurrences > 1:
                    species_counts_refine = df_refined['almindeligtNavn'].value_counts()
                    species_to_keep = species_counts_refine[species_counts_refine >= min_occurrences].index
                    df_refined = df_refined[df_refined['almindeligtNavn'].isin(species_to_keep)]
                    st.info(f"🌿 Removed {len(species_counts_refine) - len(species_to_keep)} rare species")
                
                # Apply stratified sampling
                if use_stratified and 'naturtypeId' in df_refined.columns:
                ...
                # Show new summary
                col1, col2, col3 = st.columns(3)
                col1.metric("📍 Final Plots", f"{df_refined['aktId'].nunique() if 'aktId' in df_refined.columns else 0:,}")
                col2.metric("🌿 Final Species", f"{df_refined['almindeligtNavn'].nunique() if 'almindeligtNavn' in df_refined.columns else 0:,}")
        ...
        with col1:
            ...
            if 'naturtypeId' in df_current.columns and 'aktId' in df_current.columns:
                habitat_stats = df_current.groupby('naturtypeId').agg({
                    'aktId': 'nunique',
                    'almindeligtNavn': 'nunique'
                }).reset_index()
        ...
        with col2:
            st.markdown("##### 🏷️ Major type distribution")
            if 'major_type' in df_current.columns:
                major_stats = df_current.groupby('major_type').agg({
                    'aktId': 'nunique',
                    'almindeligtNavn': 'nunique'
                }).reset_index()
                major_stats.columns = ['Major Type', 'Plot Count', 'Species Count']
                major_stats = major_stats.sort_values('Plot Count', ascending=False)
                ...
                st.warning("'major_type' column not found")
        ...
        with col1:
            st.metric("📍 Plots", f"{df_export_base['aktId'].nunique() if 'aktId' in df_export_base.columns else 0:,}")
        with col2:
            st.metric("🌿 Species", f"{df_export_base['almindeligtNavn'].nunique() if 'almindeligtNavn' in df_export_base.columns else 0:,}")
        with col3:
            st.metric("🌲 Habitats", f"{df_export_base['naturtypeId'].nunique() if 'naturtypeId' in df_export_base.columns else 0:,}")
        with col4:
        ...
        if "Geographic" in export_type:
            ...
            with col2:
                ...
                if 'UTMx' in df_export_base.columns and 'UTMy' in df_export_base.columns:
                    try:
                        unique_plots = df_export_base.drop_duplicates('aktId')[['aktId', 'UTMx', 'UTMy']]
                        unique_plots['distance'] = unique_plots.apply(
                            lambda row: calculate_distance(center_x, center_y, row['UTMx'], row['UTMy']),
                            axis=1
                        )
                        ...
                        ax.scatter(unique_plots['UTMx'], unique_plots['UTMy'], 
                        ...
                        if len(filtered_plots) > 0:
                            ax.scatter(filtered_plots['UTMx'], filtered_plots['UTMy'], 
                    ...
                    with st.spinner("Processing and exporting..."):
                        ...
                        if "Basic Export" in export_type:
                            ...
                            if 'UTMx' in df_export_base.columns and 'UTMy' in df_export_base.columns:
                                ...
                                filtered_plot_ids = unique_plots[unique_plots['distance'] <= max_distance]['aktId'].values
                                df_final = df_export_base[df_export_base['aktId'].isin(filtered_plot_ids)]
                            else:
                        ...
                        metadata = {
                            ...
                            'unique_plots': df_final['aktId'].nunique() if 'aktId' in df_final.columns else 0,
                            'unique_species': df_final['almindeligtNavn'].nunique() if 'almindeligtNavn' in df_final.columns else 0
                        }
