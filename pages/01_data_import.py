...
Part of the coTopo toolkit (Data Management 1/2)
...
def assign_eunis_major_type(habId: int) -> str:
    """Assign major habitat type based on EUNIS/Natura 2000 habitat ID codes"""
    habitat_mapping = {
        (1100, 1190): 'Marine habitats',
        (8330, 8330): 'Marine habitats',
        (1200, 1360): 'Halophytic habitats',
        (2100, 2170): 'Coastal dunes',
        (2181, 2260): 'Coastal dunes',
        (2300, 2340): 'Heath and scrub',
        (4000, 5130): 'Heath and scrub',
        (3100, 3280): 'Freshwater habitats',
        (6430, 6430): 'Freshwater habitats',
        (6100, 6500): 'Grassland',
        (7100, 7250): 'Wetland',
        (8210, 8240): 'Rocky habitats',
        (9100, 9999): 'Forests',
        (2180, 2180): 'Forests'
    }
    
    for (start, end), habitat_type in habitat_mapping.items():
        if start <= habId <= end:
            return habitat_type
    return 'Unknown type'
            ...
            if st.button("📥 Load from Source", disabled=not exists, type="primary", use_container_width=True):
                with st.spinner("Loading vegetation monitoring data..."):
                    ...
                    if df is not None:
                        ...
                        # Add major habitat type if naturtypeId exists
                        if 'naturtypeId' in df.columns:
                            df['major_type'] = df['naturtypeId'].apply(assign_eunis_major_type)
                            st.info("✅ Added major habitat type classifications")
