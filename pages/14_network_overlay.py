...
Part of the coTopo toolkit - Network-based Ecological Cartography
...
# Page configuration
st.set_page_config(
    page_title="Map Preparation - coTopo", 
    page_icon="🗺️",
    layout="wide"
)
...
def load_overlay_data(db_path: str) -> Dict[str, pd.DataFrame]:
    ...
    try:
        ...
        # Load species table
        if 'species' in tables['name'].values:
            data_dict['species'] = pd.read_sql_query('SELECT * FROM species', conn)
        
        # Load aktId table
        if 'aktId' in tables['name'].values:
            data_dict['aktId'] = pd.read_sql_query('SELECT * FROM aktId', conn)
...
def merge_species_coordinates(species_df: pd.DataFrame, coordinates_df: pd.DataFrame, 
    ...
    """Enhanced merge with flexible matching strategies and detailed reporting"""
    species_cols = set(species_df.columns)
    coord_cols = set(coordinates_df.columns)
    
    # Define merge strategies in priority order
    merge_strategies = [
        ('artId', 'artId', 'Species ID'),
        ('species', 'keyword', 'Species name → Keyword'),
        ('almindeligtNavn', 'keyword', 'Common name → Keyword'),
        ('species', 'label', 'Species name → Label'),
        ('almindeligtNavn', 'label', 'Common name → Label')
    ]
    ...
    for left_col, right_col, strategy_name in merge_strategies:
        if left_col in species_cols and right_col in coord_cols:
            # Attempt merge with indicator
            merged = pd.merge(
                species_df, 
                coordinates_df, 
                left_on=left_col, 
...
def calculate_plot_coordinates(data_df: pd.DataFrame, coordinates_df: pd.DataFrame, 
                             ...
                             aktId_df: pd.DataFrame = None) -> Tuple[pd.DataFrame, Dict]:
    """
    ...
    # Merge data with coordinates - flexible merge strategy
    if 'artId' in data_df.columns and 'artId' in coordinates_df.columns:
        merged = pd.merge(data_df, coordinates_df, on='artId', how='left', suffixes=('', '_coord'))
        calc_report['merge_method'] = 'artId'
    elif 'artsnavn' in data_df.columns and 'keyword' in coordinates_df.columns:
        merged = pd.merge(data_df, coordinates_df, left_on='artsnavn', right_on='keyword', how='left', suffixes=('', '_coord'))
        calc_report['merge_method'] = 'artsnavn → keyword'
        calc_report['warnings'].append('Using species names instead of artId (less reliable)')
    elif 'almindeligtNavn' in data_df.columns and 'keyword' in coordinates_df.columns:
        merged = pd.merge(data_df, coordinates_df, left_on='almindeligtNavn', right_on='keyword', how='left', suffixes=('', '_coord'))
        calc_report['merge_method'] = 'almindeligtNavn → keyword'
        calc_report['warnings'].append('Using Danish names instead of artId (less reliable)')
    else:
    ...
    if taxa_df is not None and not taxa_df.empty and 'artId' in taxa_df.columns and 'artId' in merged.columns:
        available_ellenberg = [col for col in ellenberg_cols if col in taxa_df.columns]
        
        if available_ellenberg:
            merged = pd.merge(merged, taxa_df[['artId'] + available_ellenberg], on='artId', how='left', suffixes=('', '_taxa'))
    ...
    if weighting == 'equal':
        plot_summary = merged_valid.groupby('aktId').agg({
            'xcoor': 'mean',
            'ycoor': 'mean',
            'artId': 'count'
        }).reset_index()
        plot_summary.rename(columns={'artId': 'speciesNum'}, inplace=True)
    ...
    if available_eco_cols:
        if weighting == 'abundance' and 'abundance' in merged_valid.columns:
            ...
            if weighted_eco:
                eco_summary = merged_valid.groupby('aktId').agg(weighted_eco).reset_index()
                abundance_summary = merged_valid.groupby('aktId')['abundance'].sum().reset_index()
                ...
                plot_summary = pd.merge(plot_summary, eco_summary, on='aktId', how='left')
            ...
            if weighted_eco:
                eco_summary = merged_valid.groupby('aktId').agg(weighted_eco).reset_index()
                degree_summary = merged_valid.groupby('aktId')['wdegree'].sum().reset_index()
            ...
            if valid_eco_cols:
                ecological_means = merged_valid.groupby('aktId')[valid_eco_cols].mean().round(2).reset_index()
                plot_summary = pd.merge(plot_summary, ecological_means, on='aktId', how='left')
    ...
    if 'mahal_dist' in merged_valid.columns and merged_valid['mahal_dist'].notna().any():
        if weighting == 'abundance' and 'abundance' in merged_valid.columns:
            ...
            mahal_num = merged_valid.groupby('aktId')['_mahal_w'].sum().reset_index()
            mahal_den = merged_valid.groupby('aktId')['abundance'].sum().reset_index()
            mahal_summary = pd.merge(mahal_num, mahal_den, on='aktId')
            mahal_summary['mean_mahal_dist'] = (mahal_summary['_mahal_w'] / mahal_summary['abundance']).round(4)
            mahal_summary = mahal_summary[['aktId', 'mean_mahal_dist']]

        elif weighting == 'degree' and 'wdegree' in merged_valid.columns:
            merged_valid['_mahal_w'] = merged_valid['mahal_dist'] * merged_valid['wdegree']
            mahal_num = merged_valid.groupby('aktId')['_mahal_w'].sum().reset_index()
            mahal_den = merged_valid.groupby('aktId')['wdegree'].sum().reset_index()
            mahal_summary = pd.merge(mahal_num, mahal_den, on='aktId')
            mahal_summary['mean_mahal_dist'] = (mahal_summary['_mahal_w'] / mahal_summary['wdegree']).round(4)
            mahal_summary = mahal_summary[['aktId', 'mean_mahal_dist']]

        else:
            mahal_summary = merged_valid.groupby('aktId')['mahal_dist'].mean().round(4).reset_index()
            mahal_summary.rename(columns={'mahal_dist': 'mean_mahal_dist'}, inplace=True)

        plot_summary = pd.merge(plot_summary, mahal_summary, on='aktId', how='left')

    # Merge with original aktId data to preserve ALL columns
    if aktId_df is not None and not aktId_df.empty:
        plot_summary = pd.merge(aktId_df, plot_summary, on='aktId', how='left', suffixes=('', '_calc'))
        calc_report['columns_preserved'] = len(aktId_df.columns)
...
def validate_map_data(taxa_df: Optional[pd.DataFrame], 
    ...
    # Check for duplicate artIds in taxa
    if taxa_df is not None and 'artId' in taxa_df.columns:
        duplicates = taxa_df['artId'].duplicated().sum()
        if duplicates > 0:
            issues.append({
                'severity': 'warning',
                'message': f'{duplicates} duplicate artIds in taxa table',
                'count': duplicates
            })
...
if 'aktId_merged' not in st.session_state:
    st.session_state.aktId_merged = None
...
if st.button("🔨 Load and Process Databases", type="primary", use_container_width=True):
    ...
    # Load additional data from session state
    taxa_df = st.session_state.get('df_taxa', None)
    ...
    with st.spinner("Calculating plot coordinates..."):
        if 'data' in overlay_data:
            aktId_df = overlay_data.get('aktId', None)
            ...
            if plot_summary.empty:
                ...
                st.session_state.aktId_merged = plot_summary
                st.session_state.calc_report = calc_report
        else:
            st.session_state.aktId_merged = None
            st.session_state.calc_report = {}
    ...
    with st.spinner("Preparing taxa table..."):
        if taxa_df is not None:
            taxa_merged = pd.merge(coordinates_df, taxa_df, on='artId', how='left', suffixes=('', '_taxa'))
        else:
...
if st.session_state.coordinates_df is not None:
    ...
    with col3:
        if st.session_state.aktId_merged is not None:
            plot_count = len(st.session_state.aktId_merged)
            st.metric("Plots processed", f"{plot_count:,}")
            
            if 'speciesNum' in st.session_state.aktId_merged.columns:
                avg_species = st.session_state.aktId_merged['speciesNum'].mean()
    ...
    with preview_tabs[1]:
        if st.session_state.aktId_merged is not None:
            ...
            st.dataframe(
                st.session_state.aktId_merged.head(preview_rows), 
                use_container_width=True,
                hide_index=True
            )
            st.caption(f"Showing {min(preview_rows, len(st.session_state.aktId_merged))} of {len(st.session_state.aktId_merged):,} plots")
            
            # Show columns
            with st.expander("📋 Available columns", expanded=False):
                st.write(list(st.session_state.aktId_merged.columns))
    ...
    with preview_tabs[3]:
        ...
        if st.session_state.aktId_merged is not None and 'xcoor' in st.session_state.aktId_merged.columns:
            plot_cov = st.session_state.aktId_merged['xcoor'].notna().sum() / len(st.session_state.aktId_merged) * 100
    ...
    if st.button("💾 Save Map Database", type="primary", use_container_width=True):
            ...
            with st.spinner("Creating map database..."):
                try:
                    ...
                    # Save plot summary
                    if st.session_state.aktId_merged is not None:
                        st.session_state.aktId_merged.to_sql('aktId', conn, if_exists='replace', index=False)
                        tables_saved.append('aktId')
                    ...
                    if create_summary_stats:
                        summary_stats = {
                            ...
                            'total_plots': len(st.session_state.aktId_merged) if st.session_state.aktId_merged is not None else 0,
                            'plots_with_coordinates': st.session_state.aktId_merged['xcoor'].notna().sum() if st.session_state.aktId_merged is not None else 0,
                            'avg_species_per_plot': st.session_state.aktId_merged['speciesNum'].mean() if st.session_state.aktId_merged is not None and 'speciesNum' in st.session_state.aktId_merged.columns else 0
                        }
                    ...
                    metadata = pd.DataFrame([{
                        ...
                        'num_plots': len(st.session_state.aktId_merged) if st.session_state.aktId_merged is not None else 0,
                        ...
                        'has_mahal_dist': 'mean_mahal_dist' in (st.session_state.aktId_merged.columns if st.session_state.aktId_merged is not None else [])
                    }])
                    ...
                    with col1:
                        ...
                        st.metric("Plots", f"{len(st.session_state.aktId_merged):,}" if st.session_state.aktId_merged is not None else "0")
