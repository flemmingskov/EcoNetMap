...
APP_INFO = {
    'title': 'coTopo - Ecological Network Mapping Toolkit',
    'version': '1.05',
    'author': 'Flemming Skov',
    'email': 'flemming.skov@ecos.au.dk',
    'github_url': 'https://github.com/flemmingskov/coTopo'
}
...
def load_settings():
    """Load and validate settings or return None if file doesn't exist"""
...
def get_project_paths():
    ...
    project_base = config.get('project', 'project_base_path', fallback=str(Path.home() / "Desktop" / "coTopo_project"))
...
# Main title
st.markdown('<h1 class="main-header">coTopo</h1>', unsafe_allow_html=True)
...
if not config:
    ...
    st.markdown("### 🚀 Welcome! Let's set up your coTopo project")
    ...
    **What we'll do:**
    ...
    default_project = str(Path.home() / "Desktop" / "coTopo_project")
    project_path = st.text_input(
        "Project folder path:",
        value=default_project,
        help="Full path where you want to store your coTopo project files"
    )
    ...
    if st.button("🔨 Create Project", type="primary", use_container_width=True):
        try:
            ...
            try:
                test_file = parent_dir / ".coTopo_write_test"
                test_file.touch()
                test_file.unlink()
            ...
            st.success(f"✅ Created project directory: {project_dir}")
            ...
            if created_folders:
                st.success(f"✅ Created folders: {', '.join(created_folders)}")
            if existing_folders:
                st.info(f"📁 Already existed: {', '.join(existing_folders)}")
            ...
            st.success(f"✅ Created {SETTINGS_FILE} file")
...
with col1:
    st.markdown("*Settings loaded from `settings.txt` in the script folder next to home.py* ")
...
with st.expander("ℹ️ About & Workflow", expanded=False):
    ...
    coTopo implements **ecological cartography** — a network-based approach to positioning
    ...
    *Skov, F. (2026). coTopo: Network-based ecological cartography for vegetation analysis.
    Aarhus University. https://github.com/flemmingskov/coTopo*
